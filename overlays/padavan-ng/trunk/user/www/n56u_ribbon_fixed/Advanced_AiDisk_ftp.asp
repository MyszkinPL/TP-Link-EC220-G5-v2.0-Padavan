	...
	for(var i = 0; i < pool_names().length; ++i){
		...
		for(var j = 0; j < permissions.length; ++j){
			...
			if(protocol == "cifs")
				...
				alert("Некорректный протокол при чтении прав доступа.");	// system error msg. must not be translate
				return;
			}
		}
	}
...
function get_permission_of_folder(accountName, poolName, folderName, protocol){
	...
	alert("Некорректное имя папки при чтении прав доступа.");	// system error msg. must not be translate
}
...
function onEvent(){
	...
	if(this.accounts.length > 0 && this.selectedAccount.length > 0 && this.selectedAccount != "anonymous"){
		...
		$("deleteAccountBtn").onclick = function(){
				if(!selectedAccount){
					alert("Учетная запись не выбрана.");
					return;
				}
	...
	if(this.selectedPool.length > 0){
		...
		$("createFolderBtn").onclick = function(){
				if(!selectedDisk){
					alert("Диск для создания общей папки не выбран.");
					return;
				}
				if(!selectedPool){
					alert("Раздел для создания общей папки не выбран.");
					return;
				}
